...
class ResistanceSimulator:
    ...
    def fetch_variant_data(self, region: str, pos: int):
        ...
        try:
            ...
            if actual_pos != pos:
                warnings.warn(
                    f"Variant at pos {pos} not found. Using closest at {actual_pos}."
                )
            ...
            warnings.warn(
                f"Cloud fetch failed for {region}: {e}. Returning random data."
            )
    ...
    def simulate_phenotypes(self, geno1: np.ndarray, geno2: np.ndarray) -> pd.Series:
        ...
        for i, sid in enumerate(self.sample_ids):
            ...
            p = (
                ...
                + has1[i] * effect1
                + has2[i] * effect2
                + country_effects.get(country, 0)
            )
            probs.append(np.clip(p, 0.01, 0.99))
        return pd.Series(
            (np.random.rand(self.n) < probs).astype(int), index=self.sample_ids
        )
