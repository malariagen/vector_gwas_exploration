...
class AnalysisHelper:
    ...
    @staticmethod
    def compute_pca_components(variant_ds: xr.Dataset, n_components: int = 5) -> pd.DataFrame:
    ...
    @staticmethod
    def prepare_modeling_dataframe(final_ds: xr.Dataset, add_pca: bool = True, n_components: int = 5) -> pd.DataFrame:
        """
        ...
        for var_name in final_ds.variants.values:
            ...
            has_variant = (final_ds['call_genotype'].sel(variants=var_name).values > 0).any(axis=1).astype(int)
